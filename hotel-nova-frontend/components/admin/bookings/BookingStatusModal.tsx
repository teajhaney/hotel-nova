    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-xl shadow-2xl w-full max-w-md animate-scale-in">
        ...
        <div className="px-6 py-4 bg-[#F8FAFC] border-b border-[#E5E7EB]">
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              <p className="text-[12px] font-semibold text-[#020887] mb-1">{booking.id}</p>
              <p className="text-[14px] font-semibold text-[#0D0F2B]">{booking.guest}</p>
              <p className="text-[13px] text-[#6B7280] mt-0.5">{booking.room}</p>
              <p className="text-[12px] text-[#9CA3AF] mt-0.5">{booking.checkIn} → {booking.checkOut}</p>
            </div>
            <div className="text-right shrink-0">
              <p className="text-[15px] font-bold text-[#0D0F2B]">₦{booking.amount.toLocaleString()}</p>
              <span className={`inline-flex items-center mt-1.5 px-2.5 py-0.5 rounded-md text-[11px] font-semibold uppercase tracking-wide ${STATUS_BADGE[booking.status] ?? 'bg-[#F1F5F9] text-[#64748B]'}`}>
                {booking.status}
              </span>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="px-6 py-5">
          ...
          {errors.status && <p className="text-[12px] text-[#EF4444] mt-1.5">{errors.status.message}</p>}
