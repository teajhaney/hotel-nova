...
export default function BookingHistoryPage() {
  ...
  const filtered = bookings?.filter((b) => {
    ...
    return STATUS_MAP[activeTab].includes(status);
  }) ?? [];
  ...
  return (
    <div className="guest-page-container pb-24 lg:pb-8">
      ...
      <div className="flex items-start justify-between mb-6 gap-4">
        <div>
          <h1 className="text-[24px] md:text-[28px] font-bold text-[#0D0F2B]">
            {GUEST_DASHBOARD_MESSAGES.historyTitle}
          </h1>
          <p className="text-[14px] text-[#64748B] mt-0.5">
            {GUEST_DASHBOARD_MESSAGES.historySubtitle}
          </p>
        </div>
      ...
      <div className="bg-white rounded-lg border border-[#E2E8F0] mb-6">
        {/* Tabs */}
        <div className="flex border-b border-[#E2E8F0] px-5 overflow-x-auto">
          {TABS.map(({ id, label }) => (
            <button
              key={id}
              onClick={() => setActiveTab(id)}
              className={`py-3.5 px-1 mr-6 text-[14px] font-medium border-b-2 transition-colors -mb-px whitespace-nowrap ${
                activeTab === id
                  ...
                  {filtered.map((booking) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-5 py-4 text-[14px] font-semibold text-[#0D0F2B]">
                          {booking.room.name}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {BOTTOM_STATS.map(({ icon: Icon, label, value }) => (
          <div
            key={label}
            className="bg-white rounded-lg border border-[#E2E8F0] p-5 flex items-center gap-4"
          >
